...
- Incident Intelligence ใช้ client-derived zones ใน Local demo เสมอ; Firebase backend จะใช้ canonical `incidentZones` แบบ realtime เมื่อ `incidentZoneSystem/state` พร้อม และ fallback กลับมาคำนวณบน client ระหว่าง backfill/stream error โดยแยก ready-empty ออกจาก not-ready ชัดเจน ดู architecture และ rollout gate ที่ `docs/INCIDENT_ZONES.md` และ `docs/INCIDENT_ZONES_RUNBOOK.md`
